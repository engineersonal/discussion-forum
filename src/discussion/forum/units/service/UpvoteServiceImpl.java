...
public class UpvoteServiceImpl implements UpvoteService {
	...
	private Upvote getUpvote(User user, Question question, Reply reply) {
		for (Upvote upvote : upvotes) {
			...
			/*
			 The below if condition explains that user can not upvote the same question or same reply twice,
			  but can upvote multiple questions and replies
			*/
			if ((upvote.getUser() == user) && (((question != null && upvote.getQuestion() == question) || (reply != null && upvote.getReply() == reply)))) {
				return upvote;
			}
		}
		return null;
